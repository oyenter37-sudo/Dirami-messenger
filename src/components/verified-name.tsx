...
export function VerifiedName({
  ...
  const hasBadges = isVerified || isHyperVerified;

  return (
    <span
      className={`inline-flex min-w-0 max-w-full items-center gap-1 overflow-visible align-middle ${className}`}
    >
      <span
        className={`${truncate ? "min-w-0 truncate" : ""} ${
          isVerified
