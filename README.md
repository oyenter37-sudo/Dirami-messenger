...
Мессенджер на Next.js 16: единый вход и регистрация через Google, приватный список чатов, запросы на общение, сообщения через **polling**, бэкенд — **Vercel Functions** (Route Handlers).
...
1. `/` — единый вход через Google. Если Google ещё не связан с Dirami, пользователь сразу выбирает свободный юз и создаёт профиль.
...
Client ID публичный и используется браузером. Client secret для этого потока не нужен: сервер проверяет подпись, `aud`, `iss` и срок действия Google ID token. Новый пользователь после Google выбирает собственный юз Dirami. Старый пользователь должен связать Google со своим текущим профилем из настроек уже открытой сессии — тогда чаты, NFT и весь прогресс остаются на прежнем внутреннем `userId`. Вход по юзу и паролю отключён.
