...
Мессенджер на Next.js 16: регистрация через Google, сохранённый вход по нику и паролю для старых аккаунтов, приватный список чатов, запросы на общение, сообщения через **polling**, бэкенд — **Vercel Functions** (Route Handlers).
...
1. `/` — вход старых пользователей по юзу и паролю; новые аккаунты создаются только через Google и после подтверждения выбирают собственный юз Dirami.
